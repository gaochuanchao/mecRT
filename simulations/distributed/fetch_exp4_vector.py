...
#
# check the impact of different sorting schemes for accessible RSUs on the distributed scheduling performance
#
import os
import re
import csv
...
INPUT_DIR = os.path.join(WORKING_DIR, "results/EXP4")
OUTPUT_DIR = os.path.join(WORKING_DIR, "result-analysis/EXP4")
# Ensure output directory exists
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Regex patterns to extract fields from filename
# file name example:  DistIS-mapScale-2-ueSortingScheme-distance.sca, DistIS-mapScale-2-ueSortingScheme-distance.vec
MATCH_PATTERN = re.compile(r'DistIS-mapScale-([0-9]+)-ueSortingScheme-([^-]+)')
...
def distributed_summary_app_count():
    ...
    params_list = ["pendingAppCount:vector", "schemeUtility:vector", "schedulingTime:vector", "measured_utility", "tokenTransferTimeMax", "tokenTransferTimeAvg", "ESexecTimeAvg"]
    ...
    with open(output_csv, mode='w', newline='') as csvfile:
        ...
        csv_writer.writerow(["mapScale", "ueSortingScheme", "time", "algorithm"] + params_list)
        ...
        for filename in os.listdir(INPUT_DIR):
            ...
            ueSortingScheme = scheAll_match.group(2) if scheAll_match else "Unknown"
            ...
            for t in timeSet:
                ...
                timeSum = sum(vector_data.get("schedulingTime:vector", {}).get(t, {}).values())
                timeCount = len(vector_data.get("schedulingTime:vector", {}).get(t, {}).values())
                data["ESexecTimeAvg"][t] = timeSum / timeCount if timeCount > 0 else 0
            ...
            for t in timeList:
                row = [mapScale, ueSortingScheme, t, "DistIS"]
