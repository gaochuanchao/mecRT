...
SCHEME_ORDER = ["SARound", "SARound-NET", "FastLR", "FastLR-NET"]
SCHEME_CHECK = {"SARound", "FastLR"}
SCHEME_NET_MAP = {
    "SARound": "SARound-NET",
    "FastLR": "FastLR-NET",
}
...
def draw_actual_energy(sche_all):
    # the actual saved energy is the difference between the actual energy and the offload energy
    analyziz = {X_NAME: [], "scheme": [], "energy": []}
    energy_file = os.path.join(FOLDER, "scheAll_" + sche_all + "_countExeTime_true_actual_energy_sum.csv")
    offload_file = os.path.join(FOLDER, "scheAll_" + sche_all + "_countExeTime_true_offload_energy_sum.csv")

    energy_file_net = os.path.join(FOLDER, "scheAll_" + sche_all + "_countExeTime_false_actual_energy_sum.csv")
    offload_file_net = os.path.join(FOLDER, "scheAll_" + sche_all + "_countExeTime_false_offload_energy_sum.csv")
    ...
    with open(energy_file_net, 'r') as f:
        # discard the first line
        f.readline()
        while True:
            line = f.readline()
    ...
    with open(offload_file_net, 'r') as f:
        # discard the first line
        f.readline()
        while True:
            line = f.readline()
    ...
    for scheme in SCHEME_ORDER:
        ...
        print(f"{scheme}: {scheme_mean * 100:.2f}%")
    ...
    ax.legend(title=LEGEND_NAME, title_fontsize=9, fontsize=9, loc="upper right", ncol=4, columnspacing=0.4, handletextpad=0.2, borderpad=0.2)
...
if __name__ == '__main__':
    ...
    print(">>> Drawing measured energy...")
    draw_actual_energy(scheAll)
